...
struct {
  ...
  struct buf buf[NBUF];
  // struct spinlock bcache_lock;  // for the whole bcache
...
binit(void)
{
  ...
  // Lazy allcation, init all the buffer to the first bucket.
...
bget(uint dev, uint blockno)
{
  ...
  // Is the block already cached in No.id cache?
  ...
  while(1) {
    i = (i + 1) % NBUCKET;
    if(i == id) continue;
    // Get the lock of the No.i cache, than read it.
    acquire(&bcache.lock[i]);
    // From the LRU
    for(b = bcache.head[i].prev; b != &bcache.head[i]; b = b->prev) {
      // If not been got before
      if(b->refcnt == 0){
        b->dev = dev;
        b->blockno = blockno;
        b->valid = 0;
